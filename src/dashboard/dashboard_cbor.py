...
def serial_reader():
    """Background thread: read CBOR packets from serial, update sensor_data & logs."""
    global start_time
    ...
    while True:
        try:
            ...
            # --- Step 4: Build package row (same as before) ---
            package_row = {
            ...
            if any(key.startswith(('BNO055_', 'BAR1_', 'BAR2_', 'LIS3DHTR_', 'GPS_', 
                                   'MainActuators_', 'DrogueActuators_', 'Buzzer_', 
                                   'Voltage_','Termoresistenze_')) for key in package_row.keys()):
                with data_lock:
            ...
            print(f"Valid CBOR received at T+{relative_time:.3f}s")
